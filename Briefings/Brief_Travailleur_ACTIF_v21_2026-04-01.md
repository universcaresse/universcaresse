...
- Risque : si INCI manquant en mémoire, peut provoquer des erreurs dans les recettes

---

## 📍 OÙ ON EST RENDU — 1 avril 2026

### ✅ Fait cette session (v21)

#### Typographie — variables `:root`
1. **Variables typographiques ajoutées au `:root`** — 7 niveaux + 2 niveaux spéciaux :
   - `--texte-micro: 0.70rem` — labels très discrets
   - `--texte-xs: 0.75rem` — sur-titres, boutons, nav
...
#### Branchements effectués (22 au total)
- `.texte-principal p` → `--texte-corps`
- `.page-hero-texte` → `--texte-corps`
- `.page-hero-titre-signature` → `--texte-xs-2`
- `.citation-source` → `--texte-xs-2`
- `.collection-tile-slogan` → `--texte-xs-2`
- `.section-count` → `--texte-sm`
- `.collection-tile-name` → `--texte-titre-sm`
- `.valeur-desc` → `--texte-corps`
- `footer` → `--texte-xs-2`
- `.page-hero-signature` → `--texte-signature`
- `.collection-entete-slogan` → `--texte-sm-2`
- `.collection-entete-desc` → `--texte-corps`
- `.ligne-groupe-desc` → `--texte-corps`
- `.carte-nom` → `--texte-corps`
- `.carte-ligne` → `--texte-xs`
- `.filtre-btn` → `--texte-xs`
- `.filtre-label` → `--texte-xs`
- `.modal-inci` → `--texte-xs`
- `.modal-collection` → `--texte-xs`
- `.modal-ligne` → `--texte-xs-2`
- `.modal-desc-ligne` → `--texte-xs-2`
- `.modal-desc` → `--texte-corps`

#### Autres corrections
- **Photo collection catalogue** — `aspect-ratio` passé de `4/5` à `1/1` (carré)
- **`.ligne-groupe-nom`** — `margin-bottom: 8px` → `0` (centré entre les deux lignes)
- **`.collection-entete-visuel` et `.collection-entete-bg`** — `aspect-ratio: 4/5` → `1/1`
- **`.carte-couleur-dot`** — bordure `2px solid var(--blanc-pur-50)` ajoutée
- **`.modal-fermer`** — bordure `2px solid var(--blanc-pur-50)` ajoutée
- **`.modal-visuel-hex`** — `align-items: center` → `align-items: flex-end` (prix en bas)
...
3. **Grossir textes lecture** — continuer à brancher sur les variables au fil des sections
4. Vérifier INCI s'affichent correctement côté public
5. Ajouter formats/prix dans `Recettes_Formats` pour les recettes qui en manquent

### ⚠️ En suspens
- `--blanc-pur-65` manquant dans le root — `.citation-source` utilise `rgba(255,255,255,0.65)` codé dur — à ajouter plus tard
- Reste encore des centaines de `font-size` codés dur dans le CSS — à brancher au fur et à mesure
- Tester visuellement tous les changements typographiques de cette session
...
1. **Grossir textes lecture site public** — en cours ✅ partiellement fait
2. Refonte visuelle — design system unique (#56)
3. Renommer `ingredient-rangee` → `form-rangee`
4. ~~Modal produit responsive iPhone/iPad~~ ✅ — portrait et paysage corrigés
5. Prix/g modal — refonte (#3)
6. Guide rapide — peaufiner (#6)
7. Taille texte mobile (#17)
8. Menu burger iPhone (#18)
9. Modal tablette (#19)
10. Taille texte mobile 16→20px (#51)
11. Spinner plus joli + partout (#61) — partiellement ✅
12. Hiérarchie typographique
13. Remplacer `<div>` par `<div class="page-entete-gauche">` dans 8 entêtes admin
14. Éliminer styles admin qui dupliquent styles publics
15. Fusionner doublons dans `style.css`
...
*Univers Caresse — Confidentiel — v21 — 1 avril 2026*
