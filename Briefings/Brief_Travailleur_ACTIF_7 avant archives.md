...
## ✅ CHANGEMENTS — SESSION 28 MARS 2026

### Page INCI — refonte visuelle complète ✅

**Fichier `style.css` :**
- `.texte-brut` — `font-size` passé de `0.8rem` à `0.95rem`
- `.form-groupe` — `margin-top: 1rem` ajouté (espace entre label et champ supérieur)
- `.accordeon-detail .form-actions` — `justify-content: space-between` (VOIR FICHE gauche, VALIDER droite)
- `.carte-admin` — nouvelle classe générique pour les cartes admin
- `.carte-admin-entete` — nouvelle classe générique pour l'entête de carte
- `.ligne-cliquable` — `cursor: pointer` + hover `--beige-15`

**Fichier `admin.js` :**
- `inciRendreLigne()` — refonte complète :
  - Ligne principale : Nom + statut (🔴/✅ sans cadre) — plus de champ INCI ni select Catégorie UC
  - Ligne entière cliquable (`ligne-cliquable`) — flèche ▼ retirée
  - Panneau détail : INCI en pleine largeur en premier, puis Catégorie UC, Catégorie fournisseur, Nom botanique, Note olfactive, Texte brut, puis `<hr>` + VOIR FICHE à gauche / VALIDER à droite
  - `thead` retiré complètement
- `inciRendreUC()` — refonte complète :
  - Tableau remplacé par cartes (`.carte-admin`) — une par catégorie
  - Ingrédients validés affichés sous chaque catégorie (depuis `listesDropdown.fullData`)
  - Boutons Modifier/Supprimer cachés si la catégorie a des ingrédients — visibles seulement si vide
  - Catégories triées par ordre alphabétique
- `inciToggleAccordeon()` — un seul accordéon ouvert à la fois — flèches retirées
- `inciConstruireAccordeons()` — flèches `inci-accord-chevron` retirées des 3 accordéons
...
- Style inline retiré sur `form-actions` : `style="padding: 0 28px 28px;"` → classe seulement
...
- `Scraping_MH`, `Scraping_Arbressence`, `Scraping_DE` — autres sources (lecture seule, mises à jour manuellement par Jean-Claude)
...
**`index_-_Admin.html`** — le panneau de contrôle de Chantal. Lu et assimilé complet session 27 mars 2026. Sections : Accueil, Collections, Recettes, Nouvelle facture, Factures, Inventaire, Contenu du site, INCI, Densités. Navigation : nav top + sidebar.
...
**`purearome.gs`** — scraping Purearome. Lance `lancerPurearome()` pour repeupler `Scraping_PA`. Un produit = une ligne = une vraie catégorie. Maintenu par le Travailleur. Les 4 sheets de scraping sont mises à jour manuellement par Jean-Claude.
...
3. Scraping peuple les sheets sources → page INCI → validation Jean-Claude → `Ingredients_INCI`
...
- Sheets de scraping mises à jour manuellement par Jean-Claude — plus de dépendance aux `.gs` pour ça
- Page INCI — ligne cliquable au lieu de flèche — un seul accordéon ouvert à la fois
- Catégories UC — boutons Modifier/Supprimer cachés si catégorie utilisée dans les ingrédients
- Catégories UC — triées alphabétiquement
