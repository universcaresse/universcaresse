...
| Fichier | État |
|---|---|
| `index.html` | ✅ lu — section médiathèque déplacée dans `<main>` v31 |
| `index_-_Admin.html` | ✅ lu — section import PDF ajoutée |
| `main.js` | ✅ lu |
| `admin.js` | ✅ lu — import PDF, page INCI restructurée, médiathèque v31 |
| `style.css` | Source unique public + admin |
| `code.gs` | ✅ + Mapping_Fournisseurs + getTexteBrut + getMappingFournisseurs + Config réinitialisé + sauvegarde auto + syncCloudinary v31 |
...
### ✅ Fait cette session (v31)
- Section `Médiathèque` déplacée à l'intérieur du `<main>` — était hors structure
- Bouton Supprimer des cartes médiathèque caché au survol (`collection-carte-actions`)
- Bouton Supprimer retiré complètement des cartes (médiathèque lecture seule)
- `syncCloudinary()` ajouté dans `code.gs` + branché dans `doGet`
- Synchro ramasse toutes les photos Cloudinary sans prefix — 157 photos
- Catégorie extraite automatiquement depuis `asset_folder`
- Bouton "↺ Synchroniser Cloudinary" ajouté dans la page Médiathèque
- `mediathequeSyncCloudinary()` ajouté dans `admin.js`
- Filtre par catégorie + compteur ajoutés dans la page Médiathèque
...
- Ménage dossiers Cloudinary en cours — 2 systèmes en parallèle pendant transition
...
*Univers Caresse — Confidentiel — v31 — 2 avril 2026*
