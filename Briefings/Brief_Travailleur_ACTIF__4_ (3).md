...
**Problème :** trop de colonnes dans le tableau, illisible. Statut Validé basé sur scraping au lieu de `Ingredients_INCI`.
...
**Fichier `admin.js` :**
- `inciRendreLigne()` — ligne principale : Nom (lecture), INCI (input), Catégorie UC (select), boutons ▼ + Valider. Panneau détail (`.accordeon-detail`) : catégorie fournisseur (lecture), nom botanique (input), note olfactive (input), texte brut (sélectionnable), lien fournisseur
- `inciToggleDetail(id)` — ouvre/ferme le panneau détail
- `inciValider()` — sauvegarde tout : INCI + catégorie UC + nom botanique + note olfactive
- `inciGetFiltres()` — lit maintenant le champ recherche en plus de statut/source
- `inciConstruireAccordeons()` — filtre par recherche + utilise `l.valide` au lieu de `l.inci`
- `inciAppliquerFiltres()` — corrigé pour ne pas planter quand appelé depuis `oninput`
- Thead mis à jour : Nom, INCI, Catégorie UC, (actions)
- Compteur "X validés" dans accordéon — basé sur `l.valide`

**Fichier `code.gs` :**
- `getSourcesInci()` — croise les lignes de scraping avec `Ingredients_INCI` : si nom trouvé → `l.valide = true`, INCI/nomBotanique/noteOlfactive pris de `Ingredients_INCI` ; sinon `l.valide = false`, INCI du scraping conservé
...
- Modal : un seul champ URL + bouton Confirmer — pas de choix de fournisseur
- `scraperIngredientUrl(url)` détecte automatiquement le fournisseur selon l'URL (logique déjà dans `scraper_url_v2.gs`)
- **Flux décidé :** les données s'inscrivent dans la sheet de scraping appropriée (PA→`Scraping_PA_v4`, MH→`Scraping_MH`, Arbressence→`Scraping_Arbressence`, DE→`Scraping_DE`, inconnu→`Scraping_Manuel`)
- La page INCI recharge — la nouvelle ligne apparaît et suit le flux normal de validation
- Chantal valide → écrit dans `Ingredients_INCI` — comme tous les autres ingrédients
- Avantage : cohérence totale du flux, tout est visible et modifiable, `Ingredients_INCI` reste la bible finale — rien n'y entre sans validation humaine

**Flux UX du modal — étape par étape :**
1. Modal s'ouvre — champ URL + bouton Confirmer
2. Chantal colle l'URL et clique Confirmer
3. Vérification doublon **avant** le scraping :
   - Trouvé dans `Ingredients_INCI` → message "Cet ingrédient existe déjà et est validé" — stop
   - Trouvé dans une sheet de scraping → message "Cet ingrédient est en attente de validation dans la page INCI" — stop
   - Rien trouvé → on procède
4. Fournisseur connu → spinner (*"C'est le temps d'un café avec ton ami. Je trouve l'information et je te dis quand c'est prêt"*) + scraping → champs remplis automatiquement
5. Fournisseur inconnu → champs vides (saisie manuelle) → ira dans `Scraping_Manuel`
6. Champ URL disparaît — champ Fournisseur affiché (nom de domaine extrait automatiquement, modifiable par Chantal) — page fournisseur s'ouvre dans un nouvel onglet (iPad friendly)
7. Modal reste ouvert dans l'admin avec : Nom, INCI, Nom botanique, Note olfactive, Texte brut, Fournisseur + bouton Valider
8. Chantal consulte l'onglet fournisseur si besoin, copie-colle ce qui manque, revient, clique Valider

**Sheet `Scraping_Manuel` :**
- Mêmes colonnes que les autres sheets de scraping
- Colonne Fournisseur = nom de domaine extrait de l'URL (ex: `purearome`, `divineessence`) — modifiable par Chantal avant confirmation
- Même flux que les autres → apparaît dans page INCI → validation Chantal → `Ingredients_INCI`
...
## 🎨 CHANTIERS VISUELS EN ATTENTE
- [ ] **Page INCI — colonnes tableau** — ajouter largeurs dans `inciRendreLigne()` : Nom 25%, INCI 35%, Catégorie UC 25%, Actions 15%
- [ ] **Animation cascade page d'accueil** — au chargement, les éléments apparaissent en séquence : Logo (déjà fait) → Accroche/titre → Bouton "Découvrez les collections" → Stats (100% Artisanal etc.). Pas un scroll fade in — une animation de chargement en cascade avec délais progressifs.
- [ ] **Mosaïque hero — tuiles dynamiques** — 3 tuiles alimentées depuis les données :
  - Tuile 1 — Disponibles : stock ≥ 1 + statut public
  - Tuile 2 — Nouveautés : date d'ajout recette — durée à définir au café avec Chantal
  - Tuile 3 — En production : dépend du module Production (à faire)

---

## 🔮 CHANTIER PRODUCTION (à planifier)
**Contexte :** Quand Chantal fabrique une recette, elle clique "Terminé" — ça déclenche tout le flux.

**Fonctionnalités décidées :**
- Date de fabrication
- Date de mise en vente calculée automatiquement (date fabrication + nb jours cure de la recette)
- Nombre de produits confectionnés
- Diminuer l'inventaire des ingrédients automatiquement
- Avertissement stock bas des ingrédients
- Avertissement nombre d'emballages nécessaires

**Calcul stock minimum — à faire après :**
- Dépend de `Ingredients_INCI` propre + recettes importées
- Stock minimum = basé sur proportions des recettes + fréquence de vente (module vente)
- Ingrédients partagés entre plusieurs recettes → besoin agrégé (ex: tournesol est partout)
- Prorata selon les ventes une fois le module vente en fonction

**Emballages — à clarifier devant l'ordi :**
- Il y a déjà une section emballages dans la recette — vérifier comment c'est branché avant de décider l'intégration dans Production

**Dépendances :**
- `Ingredients_INCI` propre ← en cours
- Import recettes ← après INCI
- Module Vente ← pour les proportions de production

---

## 🎨 CHANTIERS VISUELS EN ATTENTE
- [ ] **Page INCI — colonnes tableau** — ajouter largeurs dans `inciRendreLigne()` : Nom 25%, INCI 35%, Catégorie UC 25%, Actions 15%
- [ ] **Animation cascade page d'accueil** — au chargement : Logo (déjà fait) → Accroche/titre → Bouton "Découvrez les collections" → Stats. Pas un scroll fade in — cascade avec délais progressifs.
- [ ] **Mosaïque hero — tuiles dynamiques** — Tuile 1 : stock ≥ 1 + statut public. Tuile 2 : nouveautés (durée au café). Tuile 3 : en production (dépend module Production).
- [ ] **Entête sections admin — trop d'espace** — `.page-entete` prend trop de place sur iPad/mobile. Réduire padding/espacement pour toutes les sections admin.
- [ ] **Point couleur — contour uniforme** — appliquer le même contour partout : modal public (point + X), recettes admin (point couleur).
- [ ] **Collections — bouton "+ Nouvelle entrée"** → renommer en "+ Nouvelle collection". Disparaît quand une fiche collection EST ouverte — reste visible sur la liste. Ne devrait pas être visible quand le formulaire modification/création est ouvert.
- [ ] **Collections — formulaire modification** — titre affiche "Nouvelle collection" au lieu de "Modifier — NOM". Bouton "← Retour collection" redondant avec ✕ — à retirer.
- [ ] **Fiche collection — présentation** — revoir pour qu'elle soit aussi claire et bien structurée que la fiche recette. Même hiérarchie visuelle, même niveau de soin.
- [ ] **Flèche retour navigateur** — admin SPA sans `pushState` — retombe au login. Ajouter `history.pushState()` sur chaque `afficherSection()` et écouter `popstate`.
...
1. Page INCI — en cours 🔶
   ...
   - Liste ingrédients avec catégorie UC par ligne ✅ fait session 24 mars
   - Filtres statut/source/recherche par nom ✅ fait session 24 mars
...
- **Pousser back — obligatoire :** dire "bonne idée" et "ok" c'est facile et inutile. Si le Travailleur voit quelque chose de mieux, une incohérence, un raccourci qui va coûter cher plus tard — il le dit. Même si Jean-Claude vient de proposer l'idée. Surtout si Jean-Claude vient de proposer l'idée. Un collaborateur qui approuve tout n'est pas un collaborateur — c'est un miroir.
...
*Univers Caresse — Confidentiel — 24 mars 2026*
