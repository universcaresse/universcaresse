...
### ✅ Fait cette session (v20)
1. **Bug Modifier collection** — `fc-mode` n'était pas assigné dans `modifierCollection()` → corrigé
2. **Bug sauvegarde ligne efface photo collection** — `updateCollectionItem` en mode ligne n'écrase plus les colonnes collection — corrigé dans `code.gs` ET `admin.js`
3. **FORMAT retiré** de la fiche consultation de ligne et du formulaire de modification de ligne — `index_-_Admin.html` et `admin.js`
4. **Menu nav** — taille passée de `0.78rem` à `0.85rem`
5. **Sur-titre hero** — taille passée de `0.7rem` à `0.78rem`
6. **3e stat hero (100% ARTISANAL)** — label "Artisanal" fixé dans le HTML, retiré de Contenu du site, `—` par défaut ajouté
7. **Ombre nav au scroll** — retirée (`box-shadow: none`)
8. **Fond pages** — forcé à `#ffffff !important` sur body — crème iPad éliminé
9. **⚠️ À FAIRE** — Grossir textes de lecture sur tout le site public (en suspens — screenshots requis)
...
3. **Grossir textes lecture** — envoyer screenshots par section pour cibler
...
### ⚠️ En suspens
- Grossir textes de lecture site public — en attente de screenshots par section
...
1. **Grossir textes lecture site public** — en attente screenshots par section
...
*Univers Caresse — Confidentiel — v20 — 1 avril 2026*
