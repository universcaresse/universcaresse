...
### 🔴 PRIORITÉ — Notes olfactives PA
**Contexte :** Le texte brut de `Scraping_PA_v4` (colonne F) contient les notes olfactives mais elles ne sont pas extraites.

**Pattern trouvé :**
- Début : `Notes olfactives : `
- Fin : `. Utilisation recommandée :`

**Exemple :** `Notes olfactives : Mélange d'agrumes (citron, orange, pamplemousse) avec une pointe épicée de gingembre frais. Utilisation recommandée :`

**Chantiers à livrer (2 fichiers `.gs` séparés) :**
1. **Vérification** — scanner toutes les lignes de `Scraping_PA_v4` colonne F et confirmer que le pattern est constant partout. Livrer un rapport dans la feuille `Test_Scraper` ou Logger.
2. **Extraction** — si pattern confirmé : fonction pour extraire la note olfactive du texte brut et peupler une nouvelle colonne `Note olfactive` dans `Scraping_PA_v4` sur toutes les lignes existantes
3. **`scraper_url_v2.gs`** — corriger pour extraire la note olfactive avec ce même pattern sur les nouveaux ajouts via URL

### 🔴 PRIORITÉ — Huiles aromatiques PA (INCI à définir)
**Contexte :** Les huiles aromatiques PA sont des mélanges préparés. PA ne fournit pas le INCI. La note du texte brut ressemble à : `Nom botanique : N/A (vous référer à chaque huile de la recette) Mélange de nos huiles : (Citron, lime, orange, pamplemousse, eucalyptus globuleux)`

**Décision en attente** — à valider avec Chantal (voir document café) :
- Option 1 : `Fragrance` si moins de 1%
- Option 3 : `Blend (Citrus limon, Citrus aurantifolia...)` si plus de 1%

**Pour le Chercheur :** une fois la décision prise, livrer une fonction pour extraire les composantes du mélange depuis le texte brut et construire le INCI approprié.
...
- **Page admin correction INCI** — Travailleur doit créer une page dans l'admin pour que Chantal puisse corriger les INCI manuellement, avec champ Source, Date de correction, et lien cliquable vers la page web du produit fournisseur
- Module ajout produit temps réel — voir brief Travailleur pour le flux complet décidé
