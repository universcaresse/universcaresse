...
### ✅ Fait cette session (v24)

#### Admin — corrections bugs (session avec Claude précédent brisé)
- Collections — `id="btn-nouvelle-collection"` ajouté sur le bouton HTML
- Collections — bouton "+ Nouvelle collection" caché à l'ouverture d'une fiche (`ouvrirFicheCollection`)
- Collections — formulaire de modification affiché avant l'appel réseau `getRecettesBase`
- Recettes — `id="btn-nouvelle-recette"` ajouté sur le bouton HTML
- Recettes — `fermerFormRecette` — null check sur `.filtres-bar`
- Recettes — bouton "+ Nouvelle recette" caché à l'ouverture d'une fiche
- Recettes — ingrédients triés par poids décroissant dans le formulaire de modification
- Densités — `donneesDensites` déclarée globalement (était locale à `chargerDensites`)
...
*Univers Caresse — Confidentiel — v24 — 1 avril 2026*
