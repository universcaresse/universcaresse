...
- Redirection vers page INCI avec recherche pré-remplie ✅
- Bouton "Retour à la recette" dans page INCI ✅
- **À valider en test** : flux complet end-to-end
...
**Flux complet :**
1. Chantal dans Recettes — liste déroulante — choisit `__nouveau__`
2. Modal s'ouvre : Nom + URL fournisseur
3. Clic "Go" → sauvegarde silencieuse de la recette → scraping → écriture sheet fournisseur
4. Redirection page INCI, ingrédient pré-recherché
5. Chantal valide → `Ingredients_INCI` mis à jour
6. Bouton "← Retour à la recette" → retour avec ingrédient disponible dans liste

**Fournisseurs reconnus :** `purearome.com` → `Scraping_PA` | `lesmauvaisesherbes.com` → `Scraping_MH` | `arbressence.ca` → `Scraping_Arbressence` | `divineessence.com` → `Scraping_DE`

**Fournisseur inconnu :** ingrédient ajouté dans `Ingredients_INCI` sans scraping — call JC

**Fichiers impliqués :**
- `index_-_Admin.html` — modal `modal-ajouter-inci` + bouton `btn-retour-recette`
- `admin.js` — `ajouterIngredientInci()`, `modalInciGo()`, `fermerModalAjouterInci()`, `inciRechercher()`, `retourRecetteDepuisInci()`
- `code.gs` — `saveIngredientInci()` — écrit dans `Ingredients_INCI` + sheet fournisseur si scraped
- `scraper_url_v2.gs` — `scraperIngredientUrl()` — routeur + 4 scrapers
...
- **`scraper_url_v2.gs`** — scraping ciblé une URL à la fois — 4 fournisseurs — routeur + scrapers individuels
...
- Module ajout ingrédient via URL — scraping → sheet fournisseur → page INCI → validation → `Ingredients_INCI`
- Fournisseur inconnu dans modal ajout ingrédient → call JC
- Données scrapées (texte brut, URL) → restent dans sheets de scraping — pas dans `Ingredients_INCI`
...
- `scraper_url_v2.gs` lu et assimilé ✅ — session 28 mars 2026
