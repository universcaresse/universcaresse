...
- Catégories : Site général, Recette, Recette Noël, Collection, Collection Noël
...
### Ce que Chantal fait côté Cloudinary (avant le dev)
- Créer des dossiers : `site-general`, `recettes`, `recettes-noel`, `collections`, `collections-noel`
- Déplacer et renommer les photos existantes dans les bons dossiers
- Nommer clairement : ex. `saponica-principale`, `petit-nuage-noel`
- ⚠️ Travail en cours — 2 systèmes en parallèle pendant la transition

### Logos — fixes, on ne touche pas
- `Images/plume.png`
- `Images/Logofinal.png`

### Tâches générées (voir #76 à #82 dans la liste des tâches)
- Créer Sheet `Mediatheque` (URL, Nom, Catégorie)
- Ajouter `getMediatheque`, `saveMediatheque`, `supprimerMediatheque` dans `code.gs`
- Créer le modal sélecteur dans `index_-_Admin.html`
- Remplacer `ouvrirCloudinary()`, `ouvrirCloudinaryCollection()`, `ouvrirCloudinaryLigne()` par le nouveau sélecteur
- Ajouter clés photo dans Sheet `Contenu` pour les 4 photos éditoriales
- Brancher ces clés dans `appliquerContenu()` dans `main.js`
- Retirer les 4 URLs codées en dur de `index.html`
...
| `code.gs` | ✅ + Mapping_Fournisseurs + getTexteBrut + getMappingFournisseurs + Config réinitialisé + sauvegarde auto |
...
| `Mediatheque` | Photos Cloudinary — URL, Nom, Catégorie (à créer) |
...
### ✅ Fait cette session (v29)
- Modal médiathèque ajouté dans `index_-_Admin.html`
- Boutons "Médiathèque" ajoutés à côté des boutons Cloudinary existants (5 emplacements)
- Fonctions `ouvrirMediatheque`, `filtrerMediatheque`, `selectionnerPhotoMediatheque`, `fermerModalMediatheque` ajoutées dans `admin.js`
- Grille utilise `.collections-grille`, items `.collection-carte` + `.carte-visuel` — zéro nouveau CSS
- Branchements `getMediatheque`, `saveMediatheque`, `supprimerMediatheque` ajoutés dans `doGet` et `doPost`
- `creerSheetMediatheque()` exécuté — Sheet `Mediatheque` créée ✅
...
- Sauvegarde auto (#29) + creerDeclencheurSauvegarde() exécuté ✅
- Modal produit responsive iPhone/iPad (portrait + paysage)
- Formats dupliqués à la sauvegarde — corrigé
- Bug collections public (initSPA)
- Bug Modifier collection (fc-mode)
- Bug sauvegarde ligne efface photo collection
- Bug $/g = 0 — Config réinitialisé v27
- Listes alphabétiques formulaire facture — v27
- Harmoniser catégories Config ↔ Ingredients_INCI — v27
- Remplacer alert/confirm/prompt (#46) — partiel
- Chargement en mémoire public + admin
- Synchronisation automatique INCI/collections/catégories
- Import MD en lot (75 recettes)
- Avertissements fiche recette (INCI manquant, formats incomplets)
- console.log de debug retiré — v27
- Page INCI restructurée — charge seulement les ingrédients validés — v28
- `nomBotanique` ajouté dans `fullData` — v28
- `getTexteBrut(nom)` ajouté dans `code.gs` + branché dans `doGet` — v28
- Sheet `Mapping_Fournisseurs` créé et peuplé depuis les 4 scrapers — v28
- `getMappingFournisseurs()` ajouté dans `code.gs` + branché dans `doGet` — v28
- Section "Import PDF" ajoutée dans l'admin (sidebar Achats) — v28
- Parser Pure Arôme fonctionnel (numéro, date, items, taxes, validation totaux) — v28
- Mapping automatique PA fonctionnel — v28
- Ajout nom UC + catégorie UC à la volée depuis l'aperçu — v28
- Catégorie UC avant nom UC dans l'aperçu — filtrage par catégorie — v28

### 🔴 En cours — à terminer
- `afficherApercuItems` — version finale avec catégorie avant nom UC + filtrage + bouton "+" (trouve/remplace pas encore appliqué)
...
- Cloudinary — ménage dossiers en cours (2 systèmes en parallèle pendant transition)
...
11. ~~Restructuration page INCI~~ ✅ v28 — Select Nom UC filtré par catégorie UC — page INCI
...
32. Coût de revient complet — ingrédients + contenant + emballage + équipement amorti (#33)
33. Scan factures (#34)
34. Comptabilité (#35)
...
73. Grossir textes de lecture site public (en attente screenshots par section)
...
### 📷 MÉDIATHÈQUE PHOTOS
76. ~~Créer Sheet `Mediatheque` — colonnes : URL, Nom, Catégorie~~ ✅
77. ~~Ajouter `getMediatheque`, `saveMediatheque`, `supprimerMediatheque` dans `code.gs`~~ ✅
78. ~~Créer le modal sélecteur de photo dans `index_-_Admin.html`~~ ✅
79. ~~Remplacer `ouvrirCloudinary()`, `ouvrirCloudinaryCollection()`, `ouvrirCloudinaryLigne()` par le nouveau sélecteur interne~~ ✅ boutons ajoutés en parallèle
80. Ajouter clés photo dans Sheet `Contenu` pour les 4 photos éditoriales codées en dur
...
*Univers Caresse — Confidentiel — v29 — 2 avril 2026*
